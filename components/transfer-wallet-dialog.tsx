    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          className="h-10 rounded-xl border-border bg-background/50 hover:bg-indigo-600 hover:text-white transition-all font-black uppercase text-[10px] tracking-widest gap-2"
        >
          <IconArrowsExchange size={16} strokeWidth={3} /> Transfer Assets
        </Button>
      </DialogTrigger>
        ...
        <form action={handleSubmit} className="space-y-6 pt-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2 text-right">
              <Label className="text-[10px] font-black uppercase tracking-widest mr-1 opacity-70">
                Target Unit
              </Label>
          ...
          >
            {isPending ? (
              ...
              <>
                Initiate Transfer <IconArrowRight className="ml-2" size={16} />
              </>
            )}
          </Button>
        </form>
